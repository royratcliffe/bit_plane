...
#define D (*store) // 32-bit destination operand
...
ropRevPolish(0, 0x00U) ropRevPolish(DSon, ~(D | S)) ropRevPolish(DSna, D & ~S) ropRevPolish(Sn, ~S)
    ...
    // rop() functions execute their Boolean operation simultaneously on 32
    // bits, returning the 32-bit answer.  Static Blt-member vrop is an
